...
looks[, `:=`(lookID = NULL, StartTime = NULL, 
             ...
             anyTarget = NULL, trial = NULL)]

## Only need one set of time points for each subject (since averaged across trials)
looks <- unique(looks)

fwrite(looks, "~/dissertation/data/bob_trace_data/human_looks.csv")
...
colnames(dts) <- c("subject", "starttime", "endtime",
                   "target", "cohort")
fwrite(dts, file = "~/dissertation/data/bob_trace_data/human_saccades.csv")
...
fwrite(looks2, "~/dissertation/data/bob_trace_data/human_looks2.csv")
...
dts2 <- dts2[starttimemod <= 2300 & starttimemod >= 0, ]
...
colnames(dts2) <- c("subject", "starttime", "endtime",
                   "target", "cohort")
fwrite(dts2, file = "~/dissertation/data/bob_trace_data/human_saccades2.csv")
