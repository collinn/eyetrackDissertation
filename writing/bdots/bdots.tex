...
Like the fitting function, the bootstrapping process has been consolidated to a single function, \texttt{bdotsBoot}. In addition to taking as an argument an object returned from \texttt{bdotsFit}, \texttt{bdotsBoot} also takes a formula argument, with syntax unique to the \texttt{bdots} package. When comparing the bootstrapped differences of curves, there are two distinct types of analyses we may be interested in performing. First is a simple difference between the time series of two separate groups.
