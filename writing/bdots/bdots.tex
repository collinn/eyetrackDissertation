...
Like the fitting function, the bootstrapping process has been consolidated to a single function, \texttt{bdotsBoot}. The bootstrapping function 


In addition to taking as an argument an object returned from \texttt{bdotsFit}, \texttt{bdotsBoot} also takes a formula argument, with syntax unique to the \texttt{bdots} package.
...
\begin{center}
%\begin{verbatim}
\texttt{y} $\sim$ \texttt{group1(A, B)}
%\end{verbatim}
\end{center}

Now, it may be the case that within each group, there is a different experimental condition. For example, participants in Group1 A and B each may have been subjected to Conditions 1 and 2. In this case, we need to specify which nested Condition we are wanting to compare. To do this for the first condition, for example, we simply extend the above formula

\begin{center}
\texttt{y} $\sim$ \texttt{group1(A, B) + condition(1)}
\end{center}

This will allow us to compare the difference of curves A and B within Condition 1.

The second type of analysis we may be interested in involves a \textit{difference of differences}. For example, we may be interested (word again i know) in how the difference between Condition 1 and 2 within group A differs from the difference between Conditions 1 and 2 in group B. Here, we are still interested in comparing between groups A and B, which will remain on the right hand side of the formula. However our outcome of interest, the difference between conditions, will move to the left hand side. Maintaining that the curve is still specified by \texttt{y}, we use to \texttt{diffs} function to indicate this relationship

\begin{center}
\texttt{diffs(Condition(1, 2), y)} $\sim$ \texttt{group1(A, B)}
\end{center}
...
\begin{center}
\texttt{diffs(Condition(1, 2), y)} $\sim$ \texttt{group1(A, B) + Cohort(X)} 
\end{center}

I think maybe an actually good example here would be 

foreign/domestic $\rightarrow$ car/truck $\rightarrow$ red/bluesett
...
For example, here can show that we can have data with repeated time measures across a number of trials (saccade data). Since fitting with gnls, it's kosher. 

Can also show tumr data

\subsection{Refitting}

Here, just include a brief writeup of the refitting process
...
Idk, I actually think that this is pretty stupid, but I'll include a section of here it just to help fill space
