...
Just as with the homogeneous bootstrap, these bootstrap estimates are used to create test statistics $T_t^{(b)}$ at each time point, written
...
\caption{Observed power of each of the methods at each time in (-1,1). Might also be better to include this with subplots instead of single plot (also in ggplot2)}
